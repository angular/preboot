...
const es5DistFolder = path.join(distFolder, 'esm5');
const es2015DistFolder = path.join(distFolder, 'esm2015');
...
return Promise.resolve()
  ...
  // Compile to ES5.
  .then(() => Promise.resolve()
    .then(() => ngc(['-p', `${tempLibFolder}/tsconfig.lib.json`, '--outDir', es5OutputFolder, '--target', 'ES5']))
    .then(exitCode => exitCode === 0 ? Promise.resolve() : Promise.reject())
    .then(() => console.log('ES5 compilation succeeded.'))
  )
  // Copy typings and metadata to `dist/` folder.
  .then(() => Promise.resolve()
    .then(() => _relativeCopy('**/*.+(d.ts|metadata.json)', es2015OutputFolder, path.join(distFolder, 'typings')))
    .then(() => _metaDataReExport(distFolder, `./typings/index`, libName, libName))
    .then(() => _typingsReExport(distFolder, `./typings/index`, libName))
  ...
  .then(() => {
    ...
    const rollupBaseConfig = {
      ...
      external: [
        ...
        // See rollup/rollup/wiki/JavaScript-API#external for more.
        '@angular/animations',
        '@angular/core',
        '@angular/common',
        '@angular/http',
